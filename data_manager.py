...
from typing import Dict, List, Any, Tuple, Optional, TextIO, Union
...
def save_state_to_csv(step: int, time: float, agents: List[Agent], target: Target) -> None:
    ...
    for i, agent in enumerate(agents):
        ...
        headers: List[str] = ['Time', 'Position X', 'Position Y', 'Position Z', 'Tracking Error Norm']
...
def save_nn_to_csv(step: int, time: float, agents: List[Agent]) -> None:
    ...
    for agent in agents:
        ...
        headers: List[str] = ['Time', 'Learning Rate Spectral Norm', 'Function Approximation Error Norm', 'Neural Network Output'] + [f'Weight_{j + 1}' for j in range(len(float_weights))]
        _get_csv_writer(nn_file_path, headers, step)

        row_data: Dict[str, float] = {
            'Time': time,
            'Learning Rate Spectral Norm': float(np.max(eigvals)),
            'Function Approximation Error Norm': float(np.linalg.norm(agent.neural_network_output - agent.target.velocities[0, step - 1])),
...
def get_simulation_data() -> Tuple[List[str], List[pd.DataFrame], pd.DataFrame]:
    csv_state_files: List[str] = [f for f in os.listdir(DATA_DIR) if f.endswith(STATE_DATA_SUFFIX) and not f.startswith('target')]
    csv_state_files.sort()
    agent_types: List[str] = [f.replace(STATE_DATA_SUFFIX, '') for f in csv_state_files]
    agents_state_data: List[pd.DataFrame] = [pd.read_csv(os.path.join(DATA_DIR, f)) for f in csv_state_files]
...
def get_nn_data() -> Tuple[List[str], List[pd.DataFrame]]:
    ...
    agents_nn_data: List[pd.DataFrame] = [pd.read_csv(os.path.join(DATA_DIR, f)) for f in csv_nn_files]
...
def plot_time_series(ax: plt.Axes, time_data: pd.Series, data: pd.Series, xlabel: str, ylabel: str, title: Optional[str] = None) -> None:
...
def plot_from_csv() -> None:
    ...
    for i, pos in enumerate(agents_state_data):
        ax.plot(pos['Position X'].values, pos['Position Y'].values, pos['Position Z'].values, 
                label=agent_types[i].title(), linestyle='--', color='blue')
    ax.plot(target_state_data['Position X'], target_state_data['Position Y'], target_state_data['Position Z'], 
            label='Target Trajectory', linestyle='--', color='black')
    ...
    for i, nn in enumerate(agents_nn_data):
        fig, ax = create_plot_with_config()
        plot_time_series(ax, nn['Time'], nn['Function Approximation Error Norm'], 
                        'Time (s)', 'Function Approximation Error Norm')

    for i, nn in enumerate(agents_nn_data):
        fig, ax = create_plot_with_config()
        plot_time_series(ax, nn['Time'], nn['Learning Rate Spectral Norm'], 
                        'Time (s)', 'Learning Rate Spectral Norm')

    for i, nn in enumerate(agents_nn_data):
        fig, ax = create_plot_with_config()
        plot_time_series(ax, nn['Time'], nn['Neural Network Output'], 
                        'Time (s)', 'Neural Network Output $(m/s^2)$')

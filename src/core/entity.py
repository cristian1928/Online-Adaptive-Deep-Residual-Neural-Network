...
class Entity:
    def __init__(
        self, 
        initial_position: NDArray[np.floating[Any]], 
        time_steps: int, 
        config: dict[str, Any]
    ) -> None:
        self.num_states: int = config['num_states']
        self.time_step_delta: float = config['time_step_delta']
        self.positions: NDArray[np.floating[Any]] = np.zeros(
            (self.num_states, time_steps)
        )
        self.velocities: NDArray[np.floating[Any]] = np.zeros(
            (self.num_states, time_steps)
        )
...
class Agent(Entity):
    def __init__(
        ...
        self.neural_network_output: NDArray[np.floating[Any]] = np.zeros(
            self.num_states
        )

    def _input_func(self, step: int) -> NDArray[np.floating[Any]]: 
        return self.target.positions[:, step - 1]

    def compute_control_output(self, step: int) -> None:
        # Compute tracking error
        self.tracking_error = (
            self.target.positions[:, step - 1] - self.positions[:, step - 1]
        )
...
class Target(Entity):
    def __init__(
        ...
        self.dynamics_function: Callable[
            [NDArray[np.floating[Any]]], 
            NDArray[np.floating[Any]]
        ] = dynamics.get_dynamics_function(dynamics_type)
        
    def update_dynamics(self, step: int) -> None: 
        def dynamics_wrapper(
            t: float, 
            pos: NDArray[np.floating[Any]] | float
        ) -> NDArray[np.floating[Any]] | float:

...
>>>>>>> parent of ed2ed2d (Merge pull request #14 from cristian1928/copilot/fix-12)
    ...
    def construct_transposed_weight_matrices(self, weight_index: int) -> Tuple[int, List[np.ndarray]]:
        ...
        for rows, cols in layer_shapes:
            matrix: np.ndarray = np.array(self.weights[weight_index:weight_index + rows * cols]).reshape(rows, cols, order='F')
    ...
    def perform_forward_propagation(self, transposed_weight_matrices: List[np.ndarray], input_with_bias: np.ndarray) -> Tuple[List[np.ndarray], List[np.ndarray]]:
        ...
        for layer_index in range(self.num_layers + 1):
            unactivated_output: np.ndarray = transposed_weight_matrices[layer_index] @ activated_layers[-1]
            unactivated_layers.append(unactivated_output)
            
            if layer_index != self.num_layers:
                activation_function: str = self.outer_layer_activation_function if layer_index == self.num_layers - 1 else self.inner_layer_activation_function
    ...
    def perform_backward_propagation(self, activated_layers: List[np.ndarray], unactivated_layers: List[np.ndarray], 
                                   transposed_weight_matrices: List[np.ndarray], outer_product: Optional[np.ndarray] = None) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for layer_index in range(self.num_layers, -1, -1):
            ...
            if layer_index == self.num_layers:
                ...
                layer_gradient: np.ndarray = product @ kron_product if outer_product is None else outer_product @ product @ kron_product
...
>>>>>>> parent of ed2ed2d (Merge pull request #14 from cristian1928/copilot/fix-12)
        ...
        for block_index in range(self.num_blocks + 1):
            ...
            input_data: np.ndarray = self.get_input_with_bias(step) if block_index == 0 else self.apply_activation_function_and_bias(neural_network_output, self.shortcut_activation_function)
...
>>>>>>> parent of ed2ed2d (Merge pull request #14 from cristian1928/copilot/fix-12)
        ...
        for block_index in range(self.num_blocks, -1, -1):
            current_outer_product: Optional[np.ndarray] = outer_product if block_index < self.num_blocks else None
...
>>>>>>> parent of ed2ed2d (Merge pull request #14 from cristian1928/copilot/fix-12)
    def proj(self, Theta: np.ndarray, thetaHat: np.ndarray, thetaBar: float) -> np.ndarray:
        ...
        numerator: np.ndarray = max_term**2 * (dot_term + np.sqrt(dot_term**2 + 1.0)) * thetaHat
